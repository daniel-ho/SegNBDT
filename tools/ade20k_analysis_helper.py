...
class Dataset:
    ...
    def part_objects(self, n):
        ...
        for i in range(0, num_levels):
            thisLevel = numpy.unique(full_seg[i])
            object_names = [self.object_name(obj) for obj in thisLevel]
            print(object_names)
            print(thisLevel[1:])
            part_ids.append(thisLevel[1:])
        return part_ids 
    ...
    def get_masks(self, n):
        ...
        part_objects = self.part_objects(n)
        parts_segmentation = [self.segmentation(n)] + self.parts(n)

        # for each level of masks
        for level in range(len(part_objects)):
            # get the mask for the current level
            curr_mask = parts_segmentation[level]
            # for each object in the current level
            for obj in part_objects[level]:
                mask_new = numpy.copy(curr_mask)
                mask_new[curr_mask==obj] = 0
                mask_new[curr_mask!=obj] = 1
                mask_new = numpy.array(mask_new, dtype=numpy.uint8)

                if level == 0:
                    obj_masks[obj] = mask_new
                else:
                    part_masks[obj] = mask_new
       
        return obj_masks, part_masks 

    # both img and mask are of cv2/numpy form
